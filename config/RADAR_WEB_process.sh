...
##################
# # 시작
#./RADAR_WEB_process.sh start
# # 종료
#./RADAR_WEB_process.sh stop
# # 프로세스 확인
#./RADAR_WEB_process.sh list
##################################################
...
if [ $# == 0 ]
        then echo "Usage: RADAR_WEB_process.sh [start | stop | list | version]"; exit;
fi
 ...
 [Ss]tart)
        ...
        case "$2" in
                [Aa]ll)
                        ...
                        do
                                radar_process=`ps -ef | grep "$RADAR_NAME_JAR" | grep -v grep | wc -l`
