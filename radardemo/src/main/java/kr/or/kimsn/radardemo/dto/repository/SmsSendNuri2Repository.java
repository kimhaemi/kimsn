...
import kr.or.kimsn.radardemo.dto.SmsSendNuri2Dto;
...
//			@Param("smsFail") String smsFail,
	...
	@Query(nativeQuery = true,
			   value = "INSERT INTO nuri2.NURI2_NRMSG_DATA (\n"
						 ...
						 + "        , DATE_FORMAT(:resDate, '%Y%m%d%H%i%s') -- 발송요청시간, 예약전송:미래시간, 즉시전송: now()\n"
						 + "        , '82' -- 국가코드 : 기본 값 82, 해외 카톡 발송시 해당국가코드 입력\n"
						 + "        , :callTo -- 수신번호(숫자형태의 문자, 11~12자리),     [*][중요]반드시 휴대폰 번호 형식으로만 입력, 01X0000XXXX\n"
						 + "        , :callFrom -- 발신번호(숫자형태의 문자, 지역번호 필수), [*][중요]휴대폰번호 사용시에는 발신도용 해제 여부 필요, 스미싱 악용 방지용 '번호도용 차단서비스' 가입자는 해제 후 설정가능 합니다.\n"
