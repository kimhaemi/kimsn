SELECT * FROM (
	  ...
	  FROM (
          ...
          -- ### MO #########################################
          -- # MO 공통쿼리// MO 이용기관만 사용 합니다.
          -- ### MO #########################################
          SELECT
          'MO'            AS MSG
          , mo_key                        AS MSG_KEY             -- # '고유번호'
          , ''                            AS SUB_ID              -- # 'SUB_ID(과금청구용)'
          , ''                            AS USER_KEY            -- # '사용자 고유 번호'
          , ''                            AS USER_GROUP          -- # '사용자 그룹'
          , ''                            AS USER_ID             -- # '사용자 고유 아이디'
          , ''                            AS USER_JOBID          -- # '사용자 JOB 아이디'
          , ''                            AS CENTER_KEY          -- # '발송라인'
          , ''                            AS MSG_PRIORITY        -- # '메시지 전송 우선 순위'
          , msg_status                    AS MSG_STATE           -- # '메시지 상태 값'
          , date_mo_recv                  AS INPUT_DATE          -- # '메시지 입력시간'
          , date_mo                       AS RES_DATE            -- # '메시지 예약일시'
          , ''                            AS QUE_DATE            -- # '메시지를 수집 한 시간'
          , ''                            AS SENT_DATE           -- # '메시지를 전송 한 시간'
          , ''                            AS RSLT_DATE           -- # '핸드폰에 전달 된 시간'
          , ''                            AS REPORT_DATE         -- # 'G/W에서 결과를 수신한 시간'
          , ''                            AS RSLT_CODE           -- # '결과처리 상세코드'
          , (CASE WHEN carrier='10001' THEN 'SK' ELSE CASE WHEN carrier='10002' THEN 'KT' ELSE 'LG' END END)  AS RSLT_NET  -- # ' 최종 통신사'
          , 'MO'                            AS RSLT_TYPE           -- # '결과처리 된 메시지 유형'
          , ''                            AS SENT_COUNT          -- # '결과처리 히스토리 재전송 횟수'
          , ''                            AS HISTORY_MSG_TYPE    -- # '결과처리 히스토리 발송 타입'
          , ''                            AS HISTORY_RSLT_CODE   -- # '결과처리 히스토리 결과처리 상세코드'
          , ''                            AS IDENTIFIER          -- # '메시지 식별자코드'
          , mo_recipient||emo_recipient   AS PHONE               -- # '수신 번호'
          , mo_callback                   AS CALLBACK            -- # '발신 번호'
          , CASE WHEN service_type='5' THEN 'MMS' ELSE 'SMS' END AS MSG_TYPE_1     -- # '메시지 내용에 대한 타입 1번째'
          , (CASE WHEN service_type='5' THEN
             CASE WHEN ( SELECT IFNULL(MCNT, 0) AS MCNT FROM (
                ### mofile ###
                        SELECT mo_key, COUNT(mo_key) MCNT FROM em_mmo_file_log_202411 group by mo_key
                ### mofile ###
             ) mofile WHERE mofile.mo_key=MO.mo_key  )> 0
              THEN 'MMS' ELSE 'LMS' END
             ELSE 'SMS'
          END ) AS CONTENTS_TYPE_1                               -- # '발송 타입 1번째'
          , ''                            AS QUE_DATE_1          -- # '1번째 메시지 수집 한 시간'
          , ''                            AS SENT_DATE_1         -- # '1번째 메시지 전송 한 시간'
          , ''                            AS MSG_TYPE_2          -- # '발송 타입 2번째'
          , ''                            AS CONTENTS_TYPE_2     -- # '메시지 내용에 대한 타입 2번째'
          , ''                            AS QUE_DATE_2          -- # '2번째 메시지 수집 한 시간'
          , ''                            AS SENT_DATE_2         -- # '2번째 메시지 전송 한 시간'
          , ''                            AS MSG_TYPE_3          -- # '발송 타입 3번째'
          , ''                            AS CONTENTS_TYPE_3     -- # '메시지 내용에 대한 타입 3번째'
          , ''                            AS QUE_DATE_3          -- # '3번째 메시지 수집 한 시간'
          , ''                            AS SENT_DATE_3         -- # '3번째 메시지 전송 한 시간'
          , ''                            AS XMS_RSLT_CODE       -- # 'XMS 결과처리 상세코드'
          , ''                            AS XMS_RSLT_NET        -- # 'XMS 결과처리 통신사'
          , ''                            AS XMS_RSLT_DATE       -- # 'XMS 핸드폰에 전달 된 시간'
          , ''                            AS XMS_REPORT_DATE     -- # 'XMS G/W에서 결과를 수신한 시간'
          , ''                            AS ALT_RSLT_CODE       -- # 'ALT 결과처리 상세코드'
          , ''                            AS ALT_RSLT_NET        -- # 'ALT 결과처리 통신사'
          , ''                            AS ALT_RSLT_DATE       -- # 'ALT 핸드폰에 전달 된 시간'
          , ''                            AS ALT_REPORT_DATE     -- # 'ALT G/W에서 결과를 수신한 시간'
          , ''                            AS RCS_RSLT_CODE       -- # 'RCS 결과처리 상세코드'
          , ''                            AS RCS_RSLT_NET        -- # 'RCS 결과처리 통신사'
          , ''                            AS RCS_RSLT_DATE       -- # 'RCS 핸드폰에 전달 된 시간'
          , ''                            AS RCS_REPORT_DATE     -- # 'RCS G/W에서 결과를 수신한 시간'
          , subject                       AS XMS_SUBJECT         -- # '메시지 타이틀(LMS/MMS)'
          , content                       AS XMS_TEXT            -- # '메시지 본문(SMS/LMS/MMS)'
          , ''                            AS XMS_FILE_NAME_1     -- # '파일경로를 포함한 파일명1(MMS)'
          , ''                            AS XMS_FILE_NAME_2     -- # '파일경로를 포함한 파일명2(MMS)'
          , ''                            AS XMS_FILE_NAME_3     -- # '파일경로를 포함한 파일명3(MMS)'
          , ''                            AS ALT_COUNTRY_CODE    -- # 'ALT 국가코드(ALT,ALI)'
          , ''                            AS ALT_SENDER_KEY      -- # 'ALT 사용자 아이디(ALT,ALI)'
          , ''                            AS ALT_TEMPLATE_CODE   -- # 'ALT 등록된 템플릿 고유키(ALT,ALI)'
          , ''                            AS ALT_JSON            -- # '메시지 본문(ALT,ALI)'
          , ''                            AS RCS_BRAND_KEY       -- # 'RCS 브랜드 키'
          , ''                            AS RCS_MESSAGE_BASE_ID -- # 'RCS 메시지베이스 아이디'
          , ''                            AS RCS_JSON            -- # '메시지 본문(RCS)'
          FROM(
                ### molog ####
                 SELECT * FROM (
                         SELECT * FROM em_mo_log_202411
                ) molog
                 WHERE date_mo between '20241128000000' and  '20241128235959'
                ### molog ####
           ) MO
          -- ### MO #########################################
          -- # MO 공통쿼리
